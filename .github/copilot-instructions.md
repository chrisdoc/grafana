...
```bash
npm run build          # Compile TypeScript → dist/*.json (always run this first)
npm run fmt            # Format with Biome (100-char line width, 2-space indent)
npm run lint           # Lint with Biome
npm run deploy         # build + upload.sh (requires .env with GRAFANA_URL, GRAFANA_TOKEN)
```
